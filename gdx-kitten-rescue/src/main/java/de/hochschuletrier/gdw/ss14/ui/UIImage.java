...
public class UIImage extends Widget {
	...
	private UIButton overButton;
	
	
    Array<Drawable> overAnimation = new Array<Drawable>();
    float frameDuration;
    float stateDuration;
    int standardFrame, currentFrame, displayedFrame;
	...
	public UIImage (NinePatch patch) {
	...
	/** Creates an image stretched, and aligned center. */
	public UIImage (Skin skin, String drawableName) {
	 ...
	 * @param drawable May be null. */
	public UIImage (Drawable drawable, Scaling scaling) {
		this(drawable, scaling, Align.center);
	}

	/** @param drawable May be null. */
	public UIImage (Drawable drawable, Scaling scaling, int align) {
	...
	public void setAnimation(Skin skin, String filename, float frameDuration) {
        ...
        for (AtlasRegion atlasRegion : regions) {
            Drawable drawable = new TextureRegionDrawable(skin.getAtlas()
                    .createSprite(atlasRegion.name, atlasRegion.index));
